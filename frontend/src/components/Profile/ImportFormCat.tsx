...
const ImportFormCat = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await axiosPrivate.post('http://localhost:8080/api/categories/import', formData, {
        headers: {
  ...
  return (
    <div className="bg-white p-4 shadow" onDragOver={handleDragOver} onDrop={handleDrop}>
      <h2 className="text-xl font-bold mb-10">Εισαγωγή νέων Categories μέσω csv αρχείου</h2>
      ...
      {errorMessage && <p className="text-red-500">{errorMessage}</p>} {/* Display the error message if exists */}
      {importSuccess && <p className="text-green-500">Το αρχείο ανέβηκε με επιτυχία!</p>}
    </div>
  );
};

export default ImportFormCat;